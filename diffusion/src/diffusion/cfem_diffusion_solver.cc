...
CFEMDiffusionSolver::~CFEMDiffusionSolver() = default;
...
CFEMDiffusionSolver::Execute()
{
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    for (size_t i = 0; i < num_nodes; ++i)
    {
      ...
      for (size_t qp : fe_vol_data.GetQuadraturePointIndices())
        cell_rhs(i) += q_ext_function_(block_id_int, fe_vol_data.QPointXYZ(qp)) *
    ...
    std::vector<PetscInt> imap(num_nodes, 0); // node-mapping
    for (size_t i = 0; i < num_nodes; ++i)
      imap[i] = static_cast<PetscInt>(sdm.MapDOF(cell, i));
