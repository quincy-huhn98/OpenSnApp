...
DFEMDiffusionSolver::~DFEMDiffusionSolver() = default;
...
DFEMDiffusionSolver::SetBoundaryOptions(const InputParameters& params)
{
  ...
  if (bc_type_lc == "reflecting")
  {
    ...
    boundary_preferences_[boundary] = bndry_info;
  ...
  else if (bc_type_lc == "neumann")
  {
    ...
    BoundaryInfo bndry_info;
    bndry_info.first = BoundaryType::Neumann;
    ...
    if (boundary_preferences_.find(bndry_name) != boundary_preferences_.end())
    {
      ...
      switch (bndry_info.first)
      {
        ...
        case BoundaryType::Neumann:
        {
          ...
          boundaries_.insert(std::make_pair(
            bndry_id, Boundary{BoundaryType::Robin, {0.0, bndry_vals[1], bndry_vals[2]}}));
          log.Log() << "Boundary " << bndry_name << " set to neumann." << bndry_vals[2];
          break;
        }
...
DFEMDiffusionSolver::Execute()
{
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    const int block_id_int = static_cast<int>(block_id);

    // Assemble volumetric terms
    for (size_t i = 0; i < num_nodes; ++i)
    {
      const auto imap = static_cast<PetscInt>(sdm.MapDOF(cell, i));

      for (size_t j = 0; j < num_nodes; ++j)
      {
        const auto jmap = static_cast<PetscInt>(sdm.MapDOF(cell, j));
        double entry_aij = 0.0;
        for (size_t qp : fe_vol_data.GetQuadraturePointIndices())
        {
          entry_aij += (d_coef_function_(block_id_int, fe_vol_data.QPointXYZ(qp)) *
                          fe_vol_data.ShapeGrad(i, qp).Dot(fe_vol_data.ShapeGrad(j, qp)) +
                        sigma_a_function_(block_id_int, fe_vol_data.QPointXYZ(qp)) *
      ...
      for (size_t qp : fe_vol_data.GetQuadraturePointIndices())
        entry_rhs_i += q_ext_function_(block_id_int, fe_vol_data.QPointXYZ(qp)) *
    ...
    for (size_t f = 0; f < num_faces; ++f)
    {
      ...
      if (face.has_neighbor)
      {
        ...
        const int imat_neigh_int = static_cast<int>(imat_neigh);
        ...
        for (size_t fi = 0; fi < num_face_nodes; ++fi)
        {
          ...
          for (size_t fj = 0; fj < num_face_nodes; ++fj)
          {
            ...
            const auto jmmap = static_cast<PetscInt>(sdm.MapDOF(cell, jm));

            const int jp =
              MapFaceNodeDisc(cell, adj_cell, cc_nodes, ac_nodes, f, acf, fj); // j-plus
            const auto jpmap = static_cast<PetscInt>(sdm.MapDOF(adj_cell, jp));

            double aij = 0.0;
            for (size_t qp : fe_srf_data.GetQuadraturePointIndices())
              aij += Ckappa *
                     (d_coef_function_(block_id_int, fe_srf_data.QPointXYZ(qp)) / hm +
                      d_coef_function_(imat_neigh_int, fe_srf_data.QPointXYZ(qp)) / hp_neigh) /
        ...
        for (size_t i = 0; i < num_nodes; ++i)
        {
          ...
          for (size_t fj = 0; fj < num_face_nodes; ++fj)
          {
            ...
            Vector3 vec_aij;
            for (size_t qp : fe_srf_data.GetQuadraturePointIndices())
              vec_aij += d_coef_function_(block_id_int, fe_srf_data.QPointXYZ(qp)) *
        ...
        for (size_t fi = 0; fi < num_face_nodes; ++fi)
        {
          ...
          const auto immap = static_cast<PetscInt>(sdm.MapDOF(cell, im));

          const int ip = MapFaceNodeDisc(cell, adj_cell, cc_nodes, ac_nodes, f, acf, fi); // i-plus
          const auto ipmap = static_cast<PetscInt>(sdm.MapDOF(adj_cell, ip));
      ...
      { // boundary face
        ...
        if (bndry.type == BoundaryType::Robin)
        {
          ...
          for (size_t fi = 0; fi < num_face_nodes; ++fi)
          {
            ...
            const auto ir = static_cast<PetscInt>(sdm.MapDOF(cell, i));
            ...
            if (std::fabs(aval) >= 1.0e-12)
            {
              for (size_t fj = 0; fj < num_face_nodes; ++fj)
              {
                ...
                const auto jr = static_cast<PetscInt>(sdm.MapDOF(cell, j));
        ...
        else if (bndry.type == BoundaryType::Dirichlet)
        {
          ...
          for (size_t fi = 0; fi < num_face_nodes; ++fi)
          {
            ...
            for (size_t fj = 0; fj < num_face_nodes; ++fj)
            {
              ...
              double aij = 0.0;
              for (size_t qp : fe_srf_data.GetQuadraturePointIndices())
                aij += Ckappa * d_coef_function_(block_id_int, fe_srf_data.QPointXYZ(qp)) / hm *
            ...
            for (size_t j = 0; j < num_nodes; ++j)
            {
              ...
              for (size_t qp : fe_srf_data.GetQuadraturePointIndices())
                vec_aij += (fe_srf_data.ShapeValue(j, qp) * fe_srf_data.ShapeGrad(i, qp) +
                           ...
                           fe_srf_data.JxW(qp) *
                           d_coef_function_(block_id_int, fe_srf_data.QPointXYZ(qp));
  ...
  log.Log() << "Solving: ";
  auto petsc_solver = CreateCommonKrylovSolverSetup(
    A_, GetName(), KSPCG, PCGAMG, 0.0, residual_tolerance_, max_iters_);
